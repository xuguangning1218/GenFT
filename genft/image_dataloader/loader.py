...
def _construct_loader(dataset_dir, dataset_name, split, batch_size, shuffle, drop_last):
    ...
    if dataset_name.startswith("vtab-"):
        ...
        dataset = _DATASET_CATALOG[dataset_name](dataset_dir, dataset_name, split)
...
def construct_train_loader(dataset_dir, dataset_name, batch_size=None):
...
def construct_test_loader(dataset_dir, dataset_name, batch_size=None):
    if batch_size is None:
        bs = int(cfg.DATA.BATCH_SIZE / cfg.NUM_GPUS)
    else:
        bs = batch_size
